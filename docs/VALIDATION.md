...
- Motion-on CSS produces one fluorescent failure around the eighth second of a 10.8-second cycle. The image-slice glitch layers, moving grain, pointer response, and wordmark animation are absent; the strong static blur bloom remains part of the title in every state. Reduced-motion CSS removes the rare flicker, while the visible motion control can disable it independently.
...
- The mobile-only editorial inset frame is absent. The portrait master renders with full-bleed `cover` behavior plus a static 0.2% centered bleed, eliminating side gutters across phone aspect ratios while leaving the source master untouched and retaining zero horizontal overflow.
...
- Real screenshots are saved as `artifacts/screenshots/martyrs-desktop-1440x900.png`, `artifacts/screenshots/martyrs-mobile-390x844.png`, `artifacts/screenshots/martyrs-mobile-full-bleed-375x667.png`, and `artifacts/screenshots/martyrs-mobile-fault-390x844.png`. The short-phone capture verifies that the independent portrait master uses full-bleed `cover` behavior without side gutters.
