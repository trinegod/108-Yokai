...
- On `/portal-lab`, the final `I` in `YŌKAI` is separately wrapped with a restrained 18% right-edge clip and slightly looser tracking, keeping the intentional damaged cutoff while making the letterform legible.
- The owner-supplied Gate 02 sticker is preserved as a 2048 × 2048 transparent source master and served through separate 256- and 512-pixel transparent PNG/lossless WebP derivatives. The portal uses the same responsive marker box and accessible Gate 02 link on desktop and mobile; the yellow-versus-black art direction remains an explicit owner review decision.
