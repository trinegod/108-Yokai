...
- The speaker control and `108Y` return mark have no visible border or panel. The `108` remains black; the final `Y` uses a brighter acidic yellow at 1.6× scale with a dark 0.9-pixel keyline so it remains visible over white artwork on both breakpoints. Both controls retain accessible names and focus outlines.
