...
- The speaker control and `108Y` return mark have no visible border or panel. The `108` remains black, while the final `Y` changes only to a brighter yellow; it has no keyline, enlargement, shadow, or separate weight treatment on either breakpoint. Both controls retain accessible names and focus outlines.
...
- The Wall and Motion controls are absent. Normal motion remains active without chrome, reduced-motion remains honored, and the former `The back wall.` heading is now the smaller single-line `THE BASEMENT.`
- Local browser acceptance at 1280 × 720 and 390 × 844 confirmed zero horizontal overflow, no Wall or Motion control in the DOM, equal Gate 01/Gate 02 marker dimensions, and a readable partially clipped final `I`. On the 390-pixel phone viewport, `THE BASEMENT.` remained one line inside x=15–375.
