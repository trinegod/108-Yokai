...
- The approved 28.656708-second, stereo, 48 kHz, 16-bit PCM WAV remains copied to canonical source and preserved runtime paths byte-for-byte; both match SHA-256 `179e2748a7219f56214c5807278f0d4c7b0ccdafb37df5cc5a5b3be0eb86db9b`. A separate 56.113417-second two-pass runtime derivative circularly crossfades 1.2 seconds at the loop boundary; the left/right sample deltas across its final-to-first frame are 68/85, eliminating the master's abrupt nonzero-to-zero restart.
...
- The compact Sound control started the WAV after its click, advanced playback, and then faded to a paused state on the second click. Motion remained active without visible chrome and reduced-motion behavior remained available through the operating-system preference. No browser warning or error was recorded.
- The speaker control and `108Y` return mark have no visible border or panel. The entire `108Y` mark is black with no separate color, keyline, enlargement, shadow, or weight treatment on either breakpoint. Both controls retain accessible names and focus outlines.
- On `/portal-lab`, the final `I` in `YŌKAI` now occupies a 0.34em wrapper with only a 9% right-edge clip, retaining the damaged cutoff while exposing enough of the letterform to read clearly.
- The replacement no-target Gate 02 sticker is preserved as a 2048 × 2048 transparent source master and served through separate 256- and 512-pixel transparent PNG/lossless WebP derivatives. Gate 02 now inherits the exact same responsive marker box as Gate 01 on desktop and mobile; the superseded yellow/red master is archived but not rendered.
- The Wall and Motion controls are absent. Normal motion remains active without chrome, reduced-motion remains honored, and the chamber heading is a large centered stack with `THE` above `BASEMENT.`
- Local browser acceptance at 1280 × 720 and 390 × 844 confirmed zero horizontal overflow, the `02 / 12` hero counter, no Wall or Motion control in the DOM, equal Gate 01/Gate 02 marker dimensions, and a readable partially clipped final `I`. The centered stacked basement heading measured 283 pixels wide on the 390-pixel phone viewport, while the MART¥RS `108Y` computed to the same `rgb(9, 9, 9)` black on both breakpoints.
- Real screenshots are saved as `artifacts/screenshots/martyrs-desktop-1440x900.png`, `artifacts/screenshots/martyrs-mobile-390x844.png`, and `artifacts/screenshots/martyrs-mobile-fault-390x844.png`. The historical `martyrs-mobile-full-bleed-375x667.png` capture records the rejected enlarged `cover` comparison and is not the current mobile treatment.

## MART¥RS editorial issue — 2026-08-15
