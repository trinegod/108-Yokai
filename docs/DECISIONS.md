...
The remaining MART¥RS interface chrome is intentionally unboxed. The return link reads only `108Y`, with black numerals and a brighter yellow `Y` using the same typography, scale, weight, and spacing as the numerals; the speaker/mute control is an icon-only 44-pixel target without a visible border or panel. Both retain accessible names and keyboard focus treatment.
