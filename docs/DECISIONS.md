...
The remaining MART¥RS interface chrome is intentionally unboxed. The return link reads only `108Y`, with black numerals and an acidic-yellow `Y`; the speaker/mute control is an icon-only 44-pixel target without a visible border or panel. Both retain accessible names and keyboard focus treatment.
