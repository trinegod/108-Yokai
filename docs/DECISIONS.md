...
The owner-supplied 3840 × 2160 padded-room master is the desktop authority and the separately supplied 1440 × 3120 plate is the mobile authority. Neither is cropped, repainted, recolored, or generatively changed. Responsive `<picture>` sources select the portrait master below 700px and the wide master above it. Motion is dependency-free CSS compositing: breakpoint-specific continuous fluorescent instability, three persistently displaced image slices, a shifting wordmark blur echo, stronger moving grain, and shallow typographic pointer response. Reduced motion removes the animated atmosphere and slice layers.
