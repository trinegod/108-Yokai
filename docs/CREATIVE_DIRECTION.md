...
- Motion is limited to one rare fluorescent failure around the eighth second of a 10.8-second cycle. The title carries a strong static black blur bloom; image-slice glitches, type drift, scene warping, moving grain, and rain are absent.
...
- Audio remains optional, gesture-only, visibly controllable, locally remembered, and silent on first load. Reduced motion removes the rare flicker without removing the static title bloom, controls, or navigation.
